...
class CDrygia implements ISingleton {
	...
	public function ThemeEngineRender() {
		...
		$themePath  = DRYGIA_INSTALL_PATH . '/' . $this->config['theme']['path'];
		...
		if(isset($this->config['theme']['parent'])) {
			$parentPath = DRYGIA_INSTALL_PATH . '/' . $this->config['theme']['parent'];
			$parentUrl  = $this->request->base_url . $this->config['theme']['parent'];
		}
    
		// Add stylesheet name to the $drygia->data array
		$this->data['stylesheet'] = $this->config['theme']['stylesheet'];
		
		// Make the theme urls available as part of $drygia
	    ...
	    // Include functions.php from the parent theme if exists
		if($parentPath) {
			if(is_file("{$parentPath}/functions.php")) {
				include "{$parentPath}/functions.php";
			}
		}
		// Include current theme functions.php
	...
	public function DrawMenu($menu) {
		...
		if(isset($this->config['menus'][$menu])) {
			foreach($this->config['menus'][$menu] as $key => $val) {
				...
				$items .= "<li><a {$selected} href='" . $this->CreateUrl($val['url']) . "'>{$key}</a></li>\n";
			}
		}
		else {
